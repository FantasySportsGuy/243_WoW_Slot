...
Slot.Generator<-function(){
  ...
  reel.1<-Reel.Generator(Illidan=9,Naga=9,Gnome=7,Dwarf=5,Draenei=4,NightElf=3,
                         Human=2,Troll=2,Undead=2,Tauren=2,Orc=1,BloodElf=1,
                         Goblin=1,Naaru=0)
  reel.2<-Reel.Generator(Illidan=12,Naga=7,Gnome=5,Dwarf=3,Draenei=2,NightElf=2,
...
#Permutation is a function that calculates permutations
Permutation<-function(n,r){#calculates r permutations of k n!/((n-r)!)
  nom<-factorial(n)#numerator
  den<-factorial(n-r)#denominator
  p<-nom/den#numerator/denominator
  return(p)
}
  ...
  for(x in 1:36){#We are creating a factor table from the dataframe 
    ...
    if(x %% 3 ==1){
      ...
      not<-reel_total-(zcomb[which(zcomb$Symbol==Codex(Wins[x,3])),4+2]+zcomb[14,4+2])
         ...
         #x(s,r)=t(r)-n(s,r)
         #x(s,4)*( (x(s,4)-1)_P_2/ (t(4)-1)_P_2) is what tempmatrix[x,4] is
         tempmatrix[x,4]<-not*(Permutation(not-1,2)/Permutation(reel_total-1,2))
    ...
    if(x %% 3 ==2){
      ...
      not<-reel_total-(zcomb[which(zcomb$Symbol==Codex(Wins[x,4])),5+2]+zcomb[14,5+2])
      
      tempmatrix[x,5]<-not*(Permutation(not-1,2)/Permutation(reel_total-1,2))
    }
